...
class WrkServerHttp extends TetherWrkBase {
  ...
  init () {
    ...
    this.setInitFacs([
      ...
      ['fac', '@bitfinex/bfx-facs-db-sqlite', 'auth', 'auth', { name: 'mdk-app-node', persist: true }],
      ...
      async () => {
        ...
        if (this.ctx.orkIpc !== false) {
          ...
          try {
            ...
            debug('MDK client connected to ORK IPC: %s', sockPath)
          } catch (err) {
            debug('MDK client could not connect to ORK IPC (%s): %s', sockPath, err.message)
            this.mdkClient = null
          }
        }

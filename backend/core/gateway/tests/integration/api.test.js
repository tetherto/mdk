...
test('Api', { timeout: 90000 }, async (main) => {
  ...
  const ip = '127.0.0.1'
  const gatewayBaseUrl = `http://${ip}:${gatewayPort}`
  ...
  const createConfig = () => {
    ...
    const commonConf = { dir_log: 'logs', debug: 0, kernels: { 'cluster-1': { rpcPublicKey: '' } }, cacheTiming: {}, featureConfig: {} }
    const netConf = { r0: {} }
    const httpdConf = { h0: {} }
    ...
    const authConf = JSON.parse(fs.readFileSync(path.resolve(baseDir, '../../config/facs/auth.config.json.example')))
  ...
  const createUser = async (email, role, token) => {
    ...
    await httpClient.post(`${gatewayBaseUrl}${ENDPOINTS.USERS}`,
      {
  ...
  const minersApi = `${gatewayBaseUrl}${ENDPOINTS.MINERS}`
  ...
  await main.test('Api: get pools config by id', async (n) => {
    await testGetEndpointSecurity(n, httpClient, `${gatewayBaseUrl}${ENDPOINTS.POOLS_THING_CONFIG.replace(':id', 'miner-001')}`, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: get pools config by id - response and not found', async (n) => {
    const poolsConfigApi = (id) => `${gatewayBaseUrl}${ENDPOINTS.POOLS_THING_CONFIG.replace(':id', id)}`
  ...
  await main.test('Api: post thing-comment', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.THING_COMMENT}`
  ...
  await main.test('Api: get settings', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.SETTINGS}?rackId=1`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: put settings', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.SETTINGS}`
    const body = { rackId: 1, entries: { val: 1 } }
    await testPutEndpointSecurity(n, httpClient, api, invalidToken, body, siteOperatorUser, encoding)
  })

  await main.test('Api: get worker-config', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.WORKER_CONFIG}?type=miner`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: get thing-config', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.THING_CONFIG}?type=miner&requestType=miner`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: get tail-log', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.TAIL_LOG}?key=stat-5m`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: get tail-log/multi', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.TAIL_LOG_MULTI}?keys=[]`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: get tail-log/range-aggr', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.TAIL_LOG_RANGE_AGGR}?keys=[{"type":1,"startDate":1,"endDate":1}]`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: get history-log', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.HISTORY_LOG}?logType=alerts`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: get global/data', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.GLOBAL_DATA}?type=features`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })
  ...
  await main.test('Api: get user/settings', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.USER_SETTINGS}`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })
  ...
  await main.test('Api: get featureConfig', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.FEATURE_CONFIG}`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: get features', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.FEATURES}`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })
  ...
  await main.test('Api: get global-config', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.GLOBAL_CONFIG}`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })
  ...
  await main.test('Api: get site', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.SITE}`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: get site/status/live', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.SITE_STATUS_LIVE}`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: get site/status/live with overwriteCache', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.SITE_STATUS_LIVE}?overwriteCache=true`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })
  ...
  await main.test('Api: get permissions', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.PERMISSIONS}`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: get userinfo', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.USERINFO}`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })
  ...
  await main.test('Api: get finance/ebitda', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.FINANCE_EBITDA}?start=1700000000000&end=1700100000000`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: get finance/cost-summary', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.FINANCE_COST_SUMMARY}?start=1700000000000&end=1700100000000`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: get ext-data', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.EXT_DATA}?type=miner`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: get actions', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.ACTIONS}?queries=[]`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: get actions/batch', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.ACTIONS_BATCH}?ids=1,2`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: get actions/:type/:id', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.ACTIONS_SINGLE}`.replace(':type', 'done').replace(':id', 1)
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

  await main.test('Api: post actions/voting', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.ACTIONS_VOTING}`
    const body = { query: {}, action: '', params: [] }
    await testPostEndpointSecurity(n, httpClient, api, invalidToken, body, siteOperatorUser, encoding)
  })

  await main.test('Api: post actions/voting/batch', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.ACTIONS_VOTING_BATCH}`
  ...
  await main.test('Api: put actions/voting/:id/vote', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.ACTIONS_VOTE}`.replace(':id', 1)
  ...
  await main.test('Api: delete actions/voting/cancel', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.ACTIONS_CANCEL}?ids=1`
  ...
  await main.test('Api: post users', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.USERS}`
  ...
  await main.test('Api: post users/delete', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.USERS_DELETE}`
    const usersApi = `${gatewayBaseUrl}${ENDPOINTS.USERS}`
    const headers = await createAuthHeaders(superadminUser)
  ...
  await main.test('Api: get finance/energy-balance', async (n) => {
    const api = `${gatewayBaseUrl}${ENDPOINTS.FINANCE_ENERGY_BALANCE}?start=1700000000000&end=1700100000000`
    await testGetEndpointSecurity(n, httpClient, api, invalidToken, readonlyUser, encoding)
  })

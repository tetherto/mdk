...
test('getConfigs - invalid config type throws', async (t) => {
  const mockCtx = createMockCtxWithKernels([], async () => ([]))
...
test('getConfigs - empty kernel results', async (t) => {
  const mockCtx = createMockCtxWithKernels([{ rpcPublicKey: 'key1' }], async () => ([]))
...
test('getConfigs - handles error results from kernels', async (t) => {
  const mockCtx = createMockCtxWithKernels(
    [{ rpcPublicKey: 'key1' }],
    async () => ({ error: 'timeout' })
  )
  ...
  const result = await getConfigs(mockCtx, mockReq)
  t.ok(Array.isArray(result), 'should return array')
  t.is(result.length, 0, 'should be empty when kernel returns error')
  t.pass()
})

test('getConfigs - aggregates results from multiple kernels', async (t) => {
  let callCount = 0
  const mockCtx = createMockCtxWithKernels(
  ...
  const result = await getConfigs(mockCtx, mockReq)
  t.ok(Array.isArray(result), 'should return array')
  t.is(result.length, 2, 'should aggregate results from both kernels')
  t.pass()
})

...
const { createMockCtxWithKernels } = require('../helpers/mockHelpers')

test('flattenRpcResults - flattens multi-kernel arrays', (t) => {
  const results = [
...
test('getContainers - happy path', async (t) => {
  const mockCtx = createMockCtxWithKernels(
    [{ rpcPublicKey: 'key1' }],
    async (key, method) => method === 'getThingsCount'
      ? 2
      : [{ id: 'c1', type: 'bitdeer-d40' }, { id: 'c2', type: 'antbox-hydro' }]
  )
  ...
  t.is(result.total, 2, 'should report total from getThingsCount')
  t.pass()
})

test('getContainers - pushes filter and tag to the kernel query', async (t) => {
  let listQuery = null
  const mockCtx = createMockCtxWithKernels(
    [{ rpcPublicKey: 'key1' }],
    async (key, method, params) => {
      if (method === 'getThingsCount') return 1
      listQuery = params.query
      return [{ id: 'c1', type: 'bitdeer-d40', status: 'online' }]
    }
  )

  const result = await getContainers(mockCtx, { query: { filter: '{"status":"online"}' } })
  t.is(result.containers.length, 1, 'returns the kernel-filtered containers')
  t.is(result.total, 1, 'total reflects the filtered count')
  const serialized = JSON.stringify(listQuery)
  t.ok(serialized.includes('"status":"online"'), 'user filter is pushed to the kernel')
  t.ok(serialized.includes('t-container'), 'container tag is in the kernel query')
  t.pass()
})

test('getContainers - empty results', async (t) => {
  const mockCtx = createMockCtxWithKernels([{ rpcPublicKey: 'key1' }], async (key, method) => method === 'getThingsCount' ? 0 : [])
...
test('getCabinets - empty results', async (t) => {
  const mockCtx = createMockCtxWithKernels([{ rpcPublicKey: 'key1' }], async () => [])

...
test('aggregateRackStats - merges across multiple kernels', (t) => {
  const tailLogResults = [
    [[{
  ...
  const stats = aggregateRackStats(tailLogResults)
  t.is(stats.hashrateByRack['group-1_rack-1'], 5000000000, 'hashrate sums across kernels')
  t.is(stats.powerByRack['group-1_rack-1'], 500000, 'power sums across kernels')
  t.is(stats.efficiencyByRack['group-1_rack-1'], 7, 'efficiency takes max across kernels')
  t.pass()
})

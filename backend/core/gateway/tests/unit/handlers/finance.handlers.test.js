...
test('getEnergyBalance - empty kernel results', async (t) => {
  const mockCtx = withDataProxy({
    conf: {
      kernels: [{ rpcPublicKey: 'key1' }]
    },
...
test('processConsumptionData - processes daily data from Kernel', (t) => {
  const results = [
...
test('processCostsData - processes gateway format (energyCost)', (t) => {
  const costs = [
...
test('getEbitda - empty kernel results', async (t) => {
  const mockCtx = withDataProxy({
    conf: { kernels: [{ rpcPublicKey: 'key1' }] },
    net_r0: { jRequest: async () => ({}) },
    globalDataLib: { getGlobalData: async () => [] }
...
test('processEbitdaPrices - flat per-kernel items with priceUSD (production shape)', (t) => {
  const results = [
    [
      { ts: 1700006400000, priceUSD: 40000 },
...
test('getCostSummary - empty kernel results', async (t) => {
  const mockCtx = withDataProxy({
    conf: { kernels: [{ rpcPublicKey: 'key1' }] },
    net_r0: { jRequest: async () => ({}) },
    globalDataLib: { getGlobalData: async () => [] }
...
test('getSubsidyFees - empty kernel results', async (t) => {
  const mockCtx = withDataProxy({
    conf: { kernels: [{ rpcPublicKey: 'key1' }] },
    net_r0: { jRequest: async () => ({}) }
  })
...
test('getRevenue - empty kernel results', async (t) => {
  const mockCtx = withDataProxy({
    conf: { kernels: [{ rpcPublicKey: 'key1' }] },
    net_r0: { jRequest: async () => ({}) }
  })
...
test('getRevenueSummary - empty kernel results', async (t) => {
  const mockCtx = withDataProxy({
    conf: { kernels: [{ rpcPublicKey: 'key1' }] },
    net_r0: { jRequest: async () => ({}) },
    globalDataLib: { getGlobalData: async () => [] }
...
test('getHashRevenue - empty kernel results', async (t) => {
  const mockCtx = withDataProxy({
    conf: { kernels: [{ rpcPublicKey: 'key1' }] },
    net_r0: { jRequest: async () => ({}) }
  })
...
test('processNetworkHashrateData - flat per-kernel items (production shape)', (t) => {
  const results = [
    [

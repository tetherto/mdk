...
const { createMockCtxWithKernels, createMockReq } = require('../helpers/mockHelpers')
...
test('tailLogRoute - with invalid aggrTimes (not array)', async (t) => {
  ...
  try {
    await tailLogRoute({ conf: { kernels: [] } }, mockReq, {})
...
test('tailLogMultiRoute - with invalid keys (not array)', async (t) => {
  ...
  try {
    await tailLogMultiRoute({ conf: { kernels: [] } }, mockReq, {})

...
test('getHashrate - empty kernel results', async (t) => {
  const mockCtx = withDataProxy({
    conf: { kernels: [{ rpcPublicKey: 'key1' }] },
    net_r0: { jRequest: async () => ({}) }
  })
...
test('processHashrateData - processes array data from Kernel', (t) => {
  const results = [
...
test('processHashrateData - aggregates multiple kernels', (t) => {
  const results = [
  ...
  const daily = processHashrateData(results)
  const key = Object.keys(daily)[0]
  t.is(daily[key], 80000, 'should sum hashrate from multiple kernels')
  t.pass()
})
...
test('getConsumption - empty kernel results', async (t) => {
  const mockCtx = withDataProxy({
    conf: { kernels: [{ rpcPublicKey: 'key1' }] },
    net_r0: { jRequest: async () => ({}) }
  })
...
test('processConsumptionData - processes array data from Kernel', (t) => {
  const results = [
...
test('processConsumptionData - aggregates multiple kernels', (t) => {
  const results = [
  ...
  const daily = processConsumptionData(results)
  const key = Object.keys(daily)[0]
  t.is(daily[key], 5000, 'should sum power from multiple kernels')
  t.pass()
})
...
test('getEfficiency - empty kernel results', async (t) => {
  const mockCtx = withDataProxy({
    conf: { kernels: [{ rpcPublicKey: 'key1' }] },
    net_r0: { jRequest: async () => ({}) }
  })
...
test('processEfficiencyData - processes array data from Kernel', (t) => {
  const results = [
...
test('processEfficiencyData - averages across multiple kernels', (t) => {
  const results = [
  ...
  t.is(daily[key].count, 2, 'should track count from multiple kernels')
  t.pass()
})
...
test('getMinerStatus - empty kernel results', async (t) => {
  const mockCtx = withDataProxy({
    conf: { kernels: [{ rpcPublicKey: 'key1' }] },
    net_r0: { jRequest: async () => ({}) }
  })
...
test('processMinerStatusData - aggregates multiple kernels same day', (t) => {
  const results = [
    [{
      ts: 1700006400000,
  ...
  const daily = processMinerStatusData(results)
  const key = Object.keys(daily)[0]
  t.is(daily[key].offline, 5, 'should sum offline across kernels (3+2)')
  t.is(daily[key].sleep, 5, 'should sum sleep across kernels')
  t.is(daily[key].maintenance, 1, 'should sum maintenance across kernels')
...
test('getPowerMode - empty kernel results', async (t) => {
  const mockCtx = withDataProxy({
    conf: { kernels: [{ rpcPublicKey: 'key1' }] },
    net_r0: { jRequest: async () => ({}) }
  })
...
test('processPowerModeData - merges across multiple kernels', (t) => {
  const results = [
    [{
      ts: 1700006400000,
      power_mode_group_aggr: { 'cont1-miner1': 'normal' },
...
test('getTemperature - empty kernel results', async (t) => {
  const mockCtx = withDataProxy({
    conf: { kernels: [{ rpcPublicKey: 'key1' }] },
    net_r0: { jRequest: async () => ({}) }
  })
...
test('processContainerMiners - extracts miners from results', (t) => {
  ...
  t.is(miners.length, 2, 'should extract miners from all kernels')
  t.pass()
})

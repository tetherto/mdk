...
test('listMiners - handles empty kernel results', async (t) => {
  const ctx = createMockCtx([])
  const req = { query: {} }
...
test('listMiners - sends limit (offset+limit) to kernel data query', async (t) => {
  const capturedCalls = []
  const ctx = {
    conf: {
      featureConfig: {}
...
test('listMiners - totalCount reflects all matching items, not just overfetched page', async (t) => {
  // Simulate: 50 miners exist, but user requests offset=0, limit=5 → overfetch 5 from kernel
  // The count query returns all 50, data query returns 5 (mock returns all, but real kernel would limit)
...
test('buildKernelProjection - maps clean names to internal paths', (t) => {
  const result = buildKernelProjection({ firmware: 1, ip: 1 })
...
test('buildKernelProjection - includes sort fields for app-side sorting', (t) => {
  const userFields = { status: 1 }
  const mappedSort = { 'last.snap.stats.hashrate_mhs': -1 }
  const result = buildKernelProjection(userFields, mappedSort)
...
test('buildKernelProjection - handles multi-path fields (model needs snap.model + type)', (t) => {
  const result = buildKernelProjection({ model: 1 })
...
test('buildKernelProjection - passes through unknown field names as-is', (t) => {
  const result = buildKernelProjection({ 'some.custom.path': 1 })
...
test('listMiners - maps user fields to kernel projection and filters response', async (t) => {
  const capturedCalls = []
  ...
  // Check kernel projection was mapped correctly
  const dataCall = capturedCalls.find(c => c.method === 'listThings')
  t.is(dataCall.payload.fields['last.snap.config.firmware_ver'], 1, 'maps firmware to kernel path')
  t.is(dataCall.payload.fields['opts.address'], 1, 'maps ip to kernel path')

...
test('getPools - with filter', async (t) => {
  const mockCtx = withDataProxy({
    conf: { kernels: [{ rpcPublicKey: 'key1' }] },
    net_r0: {
      jRequest: async () => [{
        ts: '1770000000000',
        stats: [
...
test('getPools - empty kernel results', async (t) => {
  const mockCtx = withDataProxy({
    conf: { kernels: [{ rpcPublicKey: 'key1' }] },
...
const makePagedCtx = () => withDataProxy({
  conf: { kernels: [{ rpcPublicKey: 'key1' }] },
  net_r0: {
    jRequest: async () => [{
      ts: '1770000000000',
      stats: [
        { poolType: 'f2pool', username: 'alpha', hashrate: 100000, worker_count: 5, balance: 0.5 },
        { poolType: 'ocean', username: 'beta', hashrate: 200000, worker_count: 10, balance: 1.2 },
        { poolType: 'ocean', username: 'gamma', hashrate: 300000, worker_count: 15, balance: 2.4 }
      ]
    }]
  }
})

test('getPools - paginates with total over the matched set', async (t) => {
  const ctx = makePagedCtx()

  const page0 = await getPools(ctx, { query: { sort: '{"hashrate":1}', limit: 2 } }, {})
  t.alike(page0.pools.map((p) => p.account), ['alpha', 'beta'])
  t.is(page0.total, 3)
  t.is(page0.summary.poolCount, 3, 'summary stays over the full set')

  const page1 = await getPools(ctx, { query: { sort: '{"hashrate":1}', offset: 2, limit: 2 } }, {})
  t.alike(page1.pools.map((p) => p.account), ['gamma'])
  t.is(page1.total, 3)
})

test('getPools - search matches name, pool and account, total follows', async (t) => {
  const ctx = makePagedCtx()

  const byAccount = await getPools(ctx, { query: { search: 'gam' } }, {})
  t.alike(byAccount.pools.map((p) => p.account), ['gamma'])
  t.is(byAccount.total, 1)

  const byPool = await getPools(ctx, { query: { search: 'OCEAN' } }, {})
  t.is(byPool.pools.length, 2)
  t.is(byPool.total, 2)
  t.is(byPool.summary.poolCount, 3, 'summary stays over the full set')
})

test('getPools - filter and search compose before pagination', async (t) => {
  const ctx = makePagedCtx()

  const result = await getPools(ctx, { query: { query: '{"pool":"ocean"}', search: 'beta', limit: 1 } }, {})
  t.alike(result.pools.map((p) => p.account), ['beta'])
  t.is(result.total, 1)
})
...
test('flattenPoolStats - deduplicates pools across kernels', (t) => {
  const results = [
...
test('getPoolBalanceHistory - empty kernel results', async (t) => {
  const mockCtx = withDataProxy({
    conf: { kernels: [{ rpcPublicKey: 'key1' }] },
    net_r0: { jRequest: async () => ({}) }
  })

...
const makeService = (kernels, jRequestImpl) => {
  return new AlertsService({ dataProxy: buildDataProxy(kernels, jRequestImpl) })
}

test('AlertsService constructor should initialize with dataProxy', (t) => {
  const mockKernels = [{ rpcPublicKey: 'key1' }]
  const mockNet = { jRequest: async () => { } }
  const service = makeService(mockKernels, mockNet.jRequest)

  t.ok(service.dataProxy, 'Should set dataProxy property')
})

test('fetchAlerts should fetch alerts from kernels', async (t) => {
  const mockKernels = [{ rpcPublicKey: 'key1' }, { rpcPublicKey: 'key2' }]
  const mockNet = {
...
test('fetchAlerts should fetch from multiple kernels', async (t) => {
  const mockKernels = [{ rpcPublicKey: 'key1' }, { rpcPublicKey: 'key2' }, { rpcPublicKey: 'key3' }]
  let requestCount = 0
  const mockNet = {
  ...
  t.is(requestCount, 3, 'Should make requests to all kernels')
...
test('fetchAlerts should handle empty kernels array', async (t) => {
  const mockKernels = []
  const mockNet = {
    jRequest: async () => []
  }
  ...
  t.ok(Array.isArray(result), 'Should return array even with no kernels')
})

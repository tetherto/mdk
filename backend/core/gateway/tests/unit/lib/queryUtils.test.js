...
test('flattenKernelResults - flattens multiple kernel arrays', (t) => {
  const results = [[{ id: 'a' }, { id: 'b' }], [{ id: 'c' }]]
  const flat = flattenKernelResults(results)
  t.is(flat.length, 3)
  t.is(flat[0].id, 'a')
  t.is(flat[2].id, 'c')
  t.pass()
...
test('flattenKernelResults - handles empty arrays', (t) => {
  t.is(flattenKernelResults([]).length, 0)
  t.is(flattenKernelResults([[], []]).length, 0)
  t.pass()
})

test('flattenKernelResults - handles non-array kernel results', (t) => {

...
const _rpcEachLimit = async (ctx, method, params, errorHandler = null) => {
  ...
  await async.eachLimit(ctx.conf.kernels, concurrency, async (store) => {
    try {
...
const _rpcMapLimit = async (ctx, method, params) => {
  ...
  return await async.mapLimit(ctx.conf.kernels, concurrency, async (store) => {
...
const _kernelCall = async (ctx, method, params) => {
  return ctx.kernel[method](params)
}

const createDataProxy = (ctx) => {
  return {
    async requestData (method, params, errorHandler = null) {
      if (ctx.isRpcMode === false) return _kernelCall(ctx, method, params)

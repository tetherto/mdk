...
/**
 * Builds kernel projection from user-requested clean field names.
 ...
 */
function buildKernelProjection (userFields, mappedSort) {
...
async function listMiners (ctx, req) {
  ...
  const orkProjection = userFields
    ? buildKernelProjection(userFields, mappedSort)
    : MINER_DEFAULT_FIELDS
  ...
  const [kernelResults, countResults] = await Promise.all([
  ...
  let items = flattenKernelResults(kernelResults)

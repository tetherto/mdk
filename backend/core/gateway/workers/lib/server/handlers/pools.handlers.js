...
const POOL_SEARCH_FIELDS = ['name', 'pool', 'account']

function matchesPoolSearch (pool, search) {
  if (!search) return true
  const lowerSearch = search.toLowerCase()
  return POOL_SEARCH_FIELDS.some((field) => {
...
async function getPools (ctx, req) {
  ...
  let result = cursor.all()

  if (search) result = result.filter((pool) => matchesPoolSearch(pool, search))
  ...
  return { pools: result, summary, total }
}
...
function flattenPoolStats (results) {
  ...
  for (const kernelResult of results) {
    if (!kernelResult || kernelResult.error) continue
    const items = Array.isArray(kernelResult) ? kernelResult : (kernelResult.data || kernelResult.result || [])

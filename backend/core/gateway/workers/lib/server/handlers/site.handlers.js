 ...
 *
 * @param {Array} tailLogResults - Array of kernel responses from tailLogMulti
...
function aggregateMinerStats (tailLogResults) {
  ...
  for (const kernelResult of tailLogResults) {
    const entry = extractKeyEntry(kernelResult, 0)
    if (!entry) continue
...
function aggregatePowerStats (tailLogResults) {
  ...
  for (const kernelResult of tailLogResults) {
    const entry = extractKeyEntry(kernelResult, 1)
    if (!entry) continue
...
function aggregateContainerCapacity (tailLogResults) {
  ...
  for (const kernelResult of tailLogResults) {
    const entry = extractKeyEntry(kernelResult, 2)
    if (!entry) continue
 ...
 *
 * @param {Array} poolDataResults - Array of kernel responses from getWrkExtData
...
function aggregatePoolStats (poolDataResults) {
  ...
  for (const kernelResult of poolDataResults) {
    if (!Array.isArray(kernelResult)) continue
    for (const pool of kernelResult) {
 ...
 * Merges across orks (typically only 1 kernel has global config).
 *
 * @param {Array} globalConfigResults - Array of kernel responses from getGlobalConfig
...
function extractGlobalConfig (globalConfigResults) {
  ...
  for (const kernelResult of globalConfigResults) {
    if (!kernelResult || typeof kernelResult !== 'object') continue
    if (kernelResult.nominalHashrate) { config.nominalHashrate = kernelResult.nominalHashrate }
    if (kernelResult.nominalPowerAvailability_MW) {
      config.nominalPowerAvailability_MW =
        kernelResult.nominalPowerAvailability_MW
    }
  }

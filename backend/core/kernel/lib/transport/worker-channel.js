...
const debug = require('debug')('mdk:kernel:channel')
const { serialize, deserialize } = require('../protocol/envelope')

/**
 * Worker Channel
 *
 * Outbound HRPC client used by Kernel to send MDK Protocol envelopes to workers.
...
class WorkerChannel {
  constructor (opts) {
    ...
    this._rpc = opts.rpc || null // shared @hyperswarm/rpc instance (set by HRPCListener)
  }

  /**
   * Set the shared RPC instance (called by Kernel after HRPC listener starts)
   */
  setRpc (rpc) {
    this._rpc = rpc
  }

...
test('envelope - build() produces valid envelope', (t) => {
  ...
  t.is(env.sender, 'kernel:kernel:shard-1', 'has correct sender')
...
test('envelope - build() throws on missing action', (t) => {
  t.exception(() => {
    build({ type: MESSAGE_TYPES.REQUEST, sender: 'kernel' })
  }, /ERR_ENVELOPE_ACTION_REQUIRED/)
})

test('envelope - build() throws on missing type', (t) => {
  t.exception(() => {
    build({ action: ACTIONS.HEALTH_PING, sender: 'kernel' })
...
test('envelope - buildResponse() creates linked response', (t) => {
  ...
  t.is(resp.target, 'kernel:kernel:shard-1')
...
test('schemas - validatePayload command.request accepts device scope by default', (t) => {
  t.ok(validatePayload('command.request', { command: 'reboot' }).valid)
  t.ok(validatePayload('command.request', { command: 'reboot', scope: 'device' }).valid)
})

test('schemas - validatePayload command.request rejects unknown scope', (t) => {
  t.is(
    validatePayload('command.request', { command: 'reboot', scope: 'galaxy' }).error,
    'ERR_PAYLOAD_SCOPE_INVALID'
  )
})

test('schemas - validatePayload command.request requires workerId for worker/rack scope', (t) => {
  t.is(
    validatePayload('command.request', { command: 'registerThing', scope: 'worker' }).error,
    'ERR_PAYLOAD_WORKER_ID_REQUIRED'
  )
  t.is(
    validatePayload('command.request', { command: 'rackReboot', scope: 'rack' }).error,
    'ERR_PAYLOAD_WORKER_ID_REQUIRED'
  )
  t.ok(
    validatePayload('command.request', {
...
test('schemas - validatePayload command.request validates targets[]', (t) => {
  t.ok(
    validatePayload('command.request', {
  ...
  t.is(
    ...
    'ERR_PAYLOAD_TARGET_INVALID at index 1'
  )
})

test('schemas - validatePayload command.result accepts every state the CSM emits', (t) => {
  for (const status of ['QUEUED', 'DISPATCHED', 'EXECUTING', 'SUCCESS', 'FAILED', 'TIMEOUT', 'REJECTED']) {
    t.ok(validatePayload('command.result', { commandId: 'abc', status }).valid, `${status} is valid`)
  }
  t.is(
    validatePayload('command.result', { commandId: 'abc', status: 'FROBBED' }).error,
    'ERR_PAYLOAD_STATUS_INVALID'
  )
})

test('schemas - validatePayload command.status', (t) => {
  t.ok(validatePayload('command.status', { commandId: 'abc' }).valid)
  t.is(
    validatePayload('command.status', {}).error,
    'ERR_PAYLOAD_COMMAND_ID_REQUIRED'
  )
})

test('schemas - validatePayload command.status.response', (t) => {
  t.ok(
    validatePayload('command.status.response', {
      commandId: 'abc',
      status: 'QUEUED'
    }).valid
  )
  t.is(
    validatePayload('command.status.response', { commandId: 'abc' }).error,
    'ERR_PAYLOAD_STATUS_REQUIRED'
  )
  t.is(
    validatePayload('command.status.response', { commandId: 'abc', status: 'WHO_KNOWS' }).error,
    'ERR_PAYLOAD_STATUS_INVALID'
  )
})

test('schemas - validatePayload command.cancel', (t) => {
  t.ok(validatePayload('command.cancel', { commandId: 'abc' }).valid)
  t.is(
    validatePayload('command.cancel', {}).error,
    'ERR_PAYLOAD_COMMAND_ID_REQUIRED'
  )
})

test('schemas - validatePayload command.cancel.response', (t) => {
  t.ok(
    validatePayload('command.cancel.response', { commandId: 'abc', cancelled: true }).valid
  )
  t.ok(
    validatePayload('command.cancel.response', { commandId: 'abc', cancelled: false }).valid
  )
  t.is(
    validatePayload('command.cancel.response', { commandId: 'abc' }).error,
    'ERR_PAYLOAD_CANCELLED_REQUIRED'
  )
})

test('envelope - validate() accepts new command lifecycle actions', (t) => {
  for (const action of [
    ACTIONS.COMMAND_STATUS,
    ACTIONS.COMMAND_STATUS_RESPONSE,
    ACTIONS.COMMAND_CANCEL,
    ACTIONS.COMMAND_CANCEL_RESPONSE
  ]) {
    const env = build({
      action,
      type: action.endsWith('.response') ? MESSAGE_TYPES.RESPONSE : MESSAGE_TYPES.REQUEST,
      sender: 'gateway:client:1',
      payload: { commandId: 'abc', status: 'QUEUED', cancelled: false }
    })
    t.ok(validate(env).valid, `${action} envelope is valid`)
  }
})

...
const debug = require('debug')('mdk:alerts')
...
/**
 * Alert evaluation + persistence. `lib` is the alerts spec module
 * ({ specs: { <specTag>: { <alertKey>: { valid, probe } } } }) passed in
 * directly — no loadLib indirection. `listDevices()` must return
 * [{ id, last }] for saveAlerts; processThingAlerts takes one device record
 * ({ id, type, info, last }).
 */
class AlertsService {
  constructor ({ logs, lib, conf, specTags, listDevices }) {
    if (!logs) throw new Error('ERR_ALERTS_LOGS_REQUIRED')
    this.logs = logs
    this.lib = lib || null
    this.conf = conf || {}
  ...
  async saveAlerts () {
    ...
    for (const dev of this.listDevices()) {
      if (dev.last?.alerts) {
        try {
          await this._storeThgAlertsToDb(dev.id, log, dev.last.alerts)
        } catch (error) {
          debug('ERR_STORE_ALERTS_TO_DB thg:%s %s', dev.id, error.message)
        }
      }
    }

...
const { SCHEDULER_TIMES, BTC_SATS, MINUTE_MS, HOUR_MS, HOURS_24_MS } = require('./pool-utils/constants')
const { getTimeRanges } = require('./pool-utils/time')
const utilsStore = require('@tetherto/hp-svc-facs-store/utils')

/**
 * Scheduler-driven HTTP-polling pool worker base (relocated
 * MinerpoolManager). Not a Thing worker: one logical pool per instance,
 * data exposed through getWrkExtData / getWorkers rather than per-device
 * telemetry. Concrete pools (ocean, f2pool) subclass and implement the
 * fetch* hooks + getHttpUrl.
 */

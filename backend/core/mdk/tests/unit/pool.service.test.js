...
const PoolService = require('../../lib/services/pool.service')
const { SCHEDULER_TIMES, BTC_SATS, HOUR_MS } = require('../../lib/services/pool-utils/constants')
...
test('constructor throws when rack is missing', (t) => {
  t.exception(
    () => new PoolService({ wtype: WTYPE }, {}),
    /ERR_PROC_RACK_UNDEFINED/
  )
})

test('constructor throws when ctx is entirely omitted', (t) => {
  t.exception(
    () => new PoolService({ wtype: WTYPE }),
    /ERR_PROC_RACK_UNDEFINED/
  )
...
test('constructor uses a custom debugError and loadConf when provided', (t) => {
  const errors = []
  const loadConfCalls = []
  const m = new PoolService(
    { wtype: WTYPE },
    { rack: 'r1', debugError: (...args) => errors.push(args), loadConf: (...args) => loadConfCalls.push(args) }
  )
  m.debugError('boom')
  t.is(errors.length, 1)
  m.loadConf('p', 'p')
  t.is(loadConfCalls.length, 1)
})

test('default fetch* hooks are no-ops', async (t) => {
  const m = newManager()
  t.absent(await m.fetchStats(new Date()))
  t.absent(await m.fetchWorkers(new Date()))
  t.absent(await m.fetchTransactions())
  t.absent(await m.fetchBlocks())
})

test('_logErr logs to console.error without throwing', (t) => {
  const m = newManager()
  const orig = console.error
  const calls = []
  console.error = (...args) => calls.push(args)
  try {
    m._logErr('ERR_TEST', new Error('boom'))
  } finally {
    console.error = orig
  }
  t.is(calls.length, 1)
  t.is(calls[0][1], 'ERR_TEST')
})

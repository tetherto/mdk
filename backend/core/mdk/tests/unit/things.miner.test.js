...
const Miner = require('../../lib/things/miner')
...
test('default device-control methods throw Not implemented', async (t) => {
  const miner = createMiner()
  await t.exception(() => miner.powerOn(), /Not implemented/)
  await t.exception(() => miner.powerOff(), /Not implemented/)
  await t.exception(() => miner.reboot(), /Not implemented/)
  await t.exception(() => miner.setPools([]), /Not implemented/)
  await t.exception(() => miner.setFanSpeed(50), /Not implemented/)
  await t.exception(() => miner.setPowerMode('eco'), /Not implemented/)
  await t.exception(() => miner.updateFirmware('fw.bin'), /Not implemented/)
})

test('default snap-calculation hooks are no-ops', (t) => {
  const miner = createMiner()
  t.absent(miner._getStatus([], {}))
  t.absent(miner._isSuspended({}))
  t.absent(miner._calcPowerW({}))
  t.absent(miner._calcAvgTemp({}))
  t.absent(miner._getPowerMode({}))
  t.absent(miner._calcEfficiency({}, {}))
  t.absent(miner._calcHashrates({}))
})

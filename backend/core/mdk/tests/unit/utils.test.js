...
test('getRandomString: different each time', (t) => {
  const a = getRandomString(20)
  const b = getRandomString(20)
  t.not(a, b)
})
...
test('sumPoolsShares: non-array returns 0', (t) => {
  t.is(sumPoolsShares(null, 'accepted'), 0)
  t.is(sumPoolsShares(undefined, 'accepted'), 0)
})
...
test('sumPoolsShares: sums pool key', (t) => {
  ...
  t.is(sumPoolsShares(pools, 'accepted'), 18)
  t.is(sumPoolsShares(pools, 'rejected'), 3)
})
...
test('sumPoolsShares: ignores NaN', (t) => {
  ...
  t.is(sumPoolsShares(pools, 'accepted'), 15)
})

test('sumPoolsShares: empty array', (t) => {
  t.is(sumPoolsShares([], 'accepted'), 0)
})
...
test('hasErrorAndPositiveHashrate: true when errors and hashrate > 0', (t) => {
  ...
  t.ok(hasErrorAndPositiveHashrate(entry))
})
...
test('hasErrorAndPositiveHashrate: false when no errors', (t) => {
  ...
  t.not(hasErrorAndPositiveHashrate(entry))
})
...
test('hasErrorAndPositiveHashrate: safe for missing entry', (t) => {
  t.ok(!hasErrorAndPositiveHashrate(null))
  t.ok(!hasErrorAndPositiveHashrate(undefined))
  t.ok(!hasErrorAndPositiveHashrate({}))
})

...
async function createTestBee (t) {
  const tmpDir = path.join(os.tmpdir(), `mdk-ork-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`)
...
test('storage - Hyperbee sub-databases: put/get/del', async (t) => {
  ...
  const registry = bee.sub('ork-registry')
  const capabilities = bee.sub('ork-capabilities')
  const commandWal = bee.sub('ork-command-wal')
...
test('wal - append and get with real Hyperbee', async (t) => {
  const bee = await createTestBee(t)
  const walStore = bee.sub('ork-command-wal')
...
test('wal - delete removes entry', async (t) => {
  const bee = await createTestBee(t)
  const wal = new WAL(bee.sub('ork-command-wal'))
...
test('wal - crash recovery scenario: EXECUTING survives restart', async (t) => {
  ...
  // Simulate pre-crash state: a command was EXECUTING when ORK died
  ...
  // Simulate ORK restart: new WAL instance, same store

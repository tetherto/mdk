  ...
  specs: {
    ...
    miner: {
      ...
      max_outlet_temp_warning: {
        valid: (ctx, snap) => {
          return isValidSnap(snap) && !isOffline(snap) && ctx.conf.max_outlet_temp_warning && snap.stats.temperature_c?.temp?.length > 0
        },
        probe: (ctx, snap) => {
      ...
      max_pcb_temp_warning: {
        valid: (ctx, snap) => {
          return isValidSnap(snap) && !isOffline(snap) && ctx.conf.max_pcb_temp_warning && snap.stats.temperature_c?.raw_temps?.length > 0
        },
        probe: (ctx, snap) => {
      ...
      max_pcb_temp_critical: {
        valid: (ctx, snap) => {
          return isValidSnap(snap) && !isOffline(snap) && ctx.conf.max_pcb_temp_critical && snap.stats.temperature_c?.raw_temps?.length > 0
        },
        probe: (ctx, snap) => {
          const pcbs = snap.stats.temperature_c.raw_temps.map((t) => t.pcb)
          if (pcbs.length > 0) {
      ...
      max_inlet_temp_warning: {
        valid: (ctx, snap) => {
          return isValidSnap(snap) && !isOffline(snap) && ctx.conf.max_inlet_temp_warning && snap.stats.temperature_c?.temp?.length > 0
        },
        probe: (ctx, snap) => {
      ...
      max_inlet_temp_critical: {
        valid: (ctx, snap) => {
          return isValidSnap(snap) && !isOffline(snap) && ctx.conf.max_inlet_temp_critical && snap.stats.temperature_c?.temp?.length > 0
        },
        probe: (ctx, snap) => {
          const pcbInlets = snap.stats.temperature_c.temp.map((t) => t.pcbInlet)
          if (pcbInlets.length > 0) {
      ...
      min_inlet_temp_warning: {
        valid: (ctx, snap) => {
          return isValidSnap(snap) && !isOffline(snap) && ctx.conf.min_inlet_temp_warning && snap.stats.temperature_c?.temp?.length > 0
        },
        probe: (ctx, snap) => {
          const pcbInlets = snap.stats.temperature_c.temp.map((t) => t.pcbInlet)
          if (pcbInlets.length > 0) {
      ...
      min_inlet_temp_critical: {
        valid: (ctx, snap) => {
          return isValidSnap(snap) && !isOffline(snap) && ctx.conf.min_inlet_temp_critical && snap.stats.temperature_c?.temp?.length > 0
        },
        probe: (ctx, snap) => {
          const pcbInlets = snap.stats.temperature_c.temp.map((t) => t.pcbInlet)
          if (pcbInlets.length > 0) {

...
const debug = require('debug')('mdk:worker:avalon:boot')
...
/**
 * Boots an avalon worker on the WorkerRuntime. Same lifecycle model as the
 ...
 */
async function startAvalonWorker (opts) {
  ...
  const deviceType = `miner-av-${opts.model}`
  ...
  debug('avalon worker %s up: %d devices (type %s)', opts.workerId, provisioning.listDeviceIds().length, deviceType)
...
module.exports = { startAvalonWorker }

...
// mock M30S+, brings up an ORK, registers one device, prints the IDs needed for
// manual inspection, and stays running.
//
// Usage:
//   node packages/workers/miners/whatsminer/examples/run-m30sp.js
//   (Ctrl+C to stop)

const { getOrk, startWorker } = require('../../../../core/mdk')
const { WM_M30SP } = require('..')
const wmMock = require('../mock/server')

async function main () {
  wmMock.createServer({ port: 14025, host: '127.0.0.1', type: 'm30sp', serial: 'WM-001', password: 'admin' })

  const ork = await getOrk()
  const { manager } = await startWorker(WM_M30SP, { ork })

  await manager.registerThing({
    info: { container: 'site-1', serialNum: 'WM-001' },
    opts: { address: '127.0.0.1', port: 14025, password: 'admin' }
  })

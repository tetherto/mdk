...
const debug = require('debug')('mdk:worker:abb:boot')
...
const MODELS = ['b23', 'b24', 'm1m20', 'm4m20', 'reu615']
...
/**
 * Boots an ABB power-meter worker on the WorkerRuntime. Read-only device:
 * the only write actions are the provisioning builtins. Same lifecycle model
 * as the miner boots — provisioning commands mutate the persisted store and
 * take effect on the next stop() + start.
 *
 * opts:
 *   workerId    (required) one runtime process = one workerId
 *   model       (required) b23 | b24 | m1m20 | m4m20 | reu615
 ...
 */
async function startAbbWorker (opts) {
  ...
  const deviceType = `powermeter-abb-${opts.model}`
  ...
  debug('abb worker %s up: %d devices (type %s)', opts.workerId, provisioning.listDeviceIds().length, deviceType)

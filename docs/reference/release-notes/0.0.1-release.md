...
that will shape the direction of the stack. **Kernel** (`@tetherto/mdk-kernel`) lands its first reference Worker footprint here; the CLI and end-to-end developer workflows arrive
in v0.2.0–v0.3.0 (per SemVer alignment from 0.2.0 onward).

### Next release priorities

- First working version of **Kernel** (engine / orchestrator)
...
- **Kernel** (`@tetherto/mdk-kernel`) introduced with device-connectivity and data-storage interfaces, plus the canonical reference Worker line (v0.0.1 publish scopes in parentheses):
...
Kernel **`@tetherto/mdk-kernel`** — first reference Worker footprint:
...
Learn more about [Kernel](https://docs.mdk.tether.io/overview/architecture/kernel/).

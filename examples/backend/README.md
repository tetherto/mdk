...
```bash
node examples/backend/mdk-e2e/dht-ork.js
```
...
```bash
node examples/backend/mdk-e2e/client.js /tmp/mdk-dht-demo/ork.sock
```
...
### `index.js` — ORK + app-node + 4 Antminer workers

A config-driven, single-process **Antminer** site: one ORK, one HTTP app-node, and four Antminer
...
## MicroBT container site (`containers/microbt/`)

### `index.js` — ORK + 2 MicroBT container workers

A single-process **MicroBT container** site: one ORK, one HTTP app node, and two MicroBT container
workers (Kehua, Wonderint), each backed by a mock Modbus TCP device and registered as a thing.
`verify.js` exercises the live devices over the HTTP API; see
[`containers/microbt/README.md`](containers/microbt/README.md) for details.

```bash
node examples/backend/containers/microbt/index.js     # Ctrl+C to stop
node examples/backend/containers/microbt/verify.js    # in a second terminal
```
...
### `index.js` — ORK + Bitdeer D40 container worker (MQTT)

A clone-and-run **Bitdeer D40** container example: ORK + one Bitdeer worker, with a mock MQTT client
...
A clone-and-run **Seneca** sensor example: ORK + one Seneca worker backed by a mock Modbus sensor,
...
Clone-and-run power-meter examples — each brings up an ORK + one worker backed by a mock Modbus
...
`verify.js` can re-query it. Minerpools aren't wired into the ORK/MDK thing model yet, so this runs
**standalone** (no ORK/app-node) — see [`minerpools/ocean/README.md`](minerpools/ocean/README.md).
...
### `ork-shell.js` — bare ORK

Starts the ORK using the low-level `createORK()` directly, without `getOrk()`. Useful for seeing the raw configuration shape before any bootstrap helpers are applied.

```bash
node examples/backend/ork/ork-shell.js
```

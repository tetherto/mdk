 ...
 *
 * Run:
 *   node backend/core/kernel/examples/telemetry-flow.js
 *
...
async function main () {
  ...
  // Subscribe: print a summary line on every scheduler-driven pull. The
  // runtime's metrics payload is a channel map; the full legacy snapshot
  // rides on the `snap` channel.
  kernel.telemetryCollector.subscribe(deviceId, (data) => {
    if (!data || !data.metrics) return
    const s = (data.metrics.snap && data.metrics.snap.stats) || {}

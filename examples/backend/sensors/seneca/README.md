...
hardware**. It starts a mock Seneca sensor, brings up an ORK, registers the sensor as a thing, and
...
- Bringing up an ORK and one Seneca worker in a single process.
- Starting a **mock** Seneca sensor (Modbus TCP) and **registering** it as a thing.
- Pulling live temperature telemetry through the ORK over the MDK Protocol — no hardware.
...
Pull live telemetry for the sensor (this is the line `index.js` prints, with the real key/device filled in):
...
# → {"deviceId":"...","metrics":{"stats":{"status":"ok","temp_c":33.2}},...}
```
...
2. Brings up an ORK (HRPC only) pinned to its own store root (`$TMPDIR/mdk-site-seneca/ork/`).
3. `startWorker(SENECA, { ork })` and `manager.registerThing({ info, opts })`.
...
├── README.md
├── package.json
├── index.js                      # ORK + Seneca worker + mock + registration
└── .gitignore
```
...
```
$TMPDIR/mdk-site-seneca/ork/      # ORK Corestore
```

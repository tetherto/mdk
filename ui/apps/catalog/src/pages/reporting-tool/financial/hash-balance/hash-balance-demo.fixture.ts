import type { HashRevenueLogEntry } from '@tetherto/mdk-react-devkit/domain'

export type HashBalanceMonthSnapshot = {
...
export const HASH_BALANCE_MONTHLY: Record<number, HashBalanceMonthSnapshot[]> = {
  2025: [
    {
...
export const buildDemoLogEntry = (
  year: number,
  month: number,
  snapshot: HashBalanceMonthSnapshot,
): HashRevenueLogEntry => {
  const {
...
export const buildDemoYearLog = (year: number): HashRevenueLogEntry[] => {
  const months = HASH_BALANCE_MONTHLY[year]
  if (!months) return []
  ...
  return months
    .slice(0, lastMonth)
    .map((snapshot, index) => buildDemoLogEntry(year, index + 1, snapshot))
}

...
/**
 * Snapshot of the pagination state owned by {@link usePagination}.
 *
 * Mirrors the props expected by the devkit `<Pagination>` component so it
 * can be spread directly: `<Pagination {...pagination} onChange={...} />`.
 */
export type PaginationState = {
  ...
  pageSize: number
  /** Whether the UI should expose a page-size selector. */
  showSizeChanger: boolean
  /** Total row count (may be padded when unknown — see `hideNextPage`). */
  total: number
}

...
/**
 * Derives the static IP address a miner should receive based on its
 * physical position (container number + socket coordinates).
 *
 * Returns an empty string when the feature is disabled or the socket is
 * incomplete, so callers can drop the value straight into a form field.
 *
 * @category ui
 */

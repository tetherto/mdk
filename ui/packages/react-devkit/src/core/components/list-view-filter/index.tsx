...
import { forwardRef, type JSX, useCallback, useEffect, useMemo, useState } from 'react'
 ...
 * @tier agent-ready
 */
const ListViewFilter = forwardRef<HTMLDivElement, ListViewFilterProps>(
  ({ options, onChange, localFilters, className, filterKey = 'default' }, ref) => {
    ...
    /**
     * Close the popover when the user scrolls outside it. Without this the
     * Radix Popper recalculates its position on every scroll frame and the
     * dropdown appears to "jump" along with the page.
     */
    useEffect(() => {
      if (!open) return undefined
      const handleScroll = (event: Event): void => {
        const target = event.target as Node | null
        const dropdown = document.querySelector('.mdk-list-view-filter__dropdown')

...
type SelectContextValue = Partial<{
  value: string
  allowClear: boolean
...
/**
 * Dropdown select built on Radix UI. Supports `default` and `colored`
 * variants, multiple sizes, and full keyboard navigation.
 *
 * @category forms
 * @domain generic
 * @tier agent-ready
...
const Select = (allProps: SelectProps) => {
  const { allowClear, onValueChange, value, defaultValue, children, ...props } = allProps
  const [internalValue, setInternalValue] = useState<string | undefined>(defaultValue)
  const isControlled = 'value' in allProps
  const currentValue = isControlled ? value : internalValue
  /** Radix treats `value={undefined}` as uncontrolled (`prop === undefined`); use '' for controlled “no value”. */
  const resolvedValue = isControlled ? currentValue : internalValue
  const radixValue = isControlled ? (resolvedValue ?? '') : internalValue
  ...
  return (
    <SelectContext.Provider value={{ value: currentValue, onClear: handleClear, allowClear }}>
      <SelectPrimitive.Root value={radixValue} onValueChange={handleValueChange} {...props}>
        {children}
...
const SelectTrigger = forwardRef<ComponentRef<typeof SelectPrimitive.Trigger>, SelectTriggerProps>(
  ({ className, children, size = 'lg', variant = 'default', color, style, ...props }, ref) => {
    const { value, onClear, allowClear } = useContext(SelectContext)
...
/**
 * Floating dropdown panel inside `<Select>` that hosts the list of `<SelectItem>`s.
 * @category forms
 * @domain generic
 * @tier internal
 */
...
>(
  (
    { className, children, position = 'popper', sideOffset = 8, collisionPadding = 16, ...props },
    ref,
  ) => (
    <SelectPrimitive.Portal>
      ...
      >
        <SelectPrimitive.Viewport
          className={cn(
            position === 'popper' && 'mdk-select__viewport mdk-select__viewport--popper',
          )}
        >
          {children}
        </SelectPrimitive.Viewport>
...
/**
 * Single selectable option rendered inside `<SelectContent>` — pass `value` for the option key and children for the label.
 * @category forms
 * @domain generic
 * @tier internal
 */

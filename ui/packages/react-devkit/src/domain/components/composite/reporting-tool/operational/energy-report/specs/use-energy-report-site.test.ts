...
describe('useEnergyReportSite', () => {
  it('returns expected shape', () => {
    ...
    expect(result.current.tailLogHead).toBeUndefined()
    expect(result.current.miningUnitCards).toEqual([])
  ...
  it('derives tailLogHead and miningUnitCards from tail log and containers', () => {
    const tailLog = [
      [
        {
          hashrate_mhs_5m_active_container_group_cnt: { 'container-1': 3 },
        ...
        containers: [
          {
            ...
            info: { container: 'container-1', nominalMinerCapacity: 10 },
          },
        ],
      }),
    )
    expect(result.current.tailLogHead).toEqual(tailLog[0]![0])
    expect(result.current.miningUnitCards).toHaveLength(1)
    expect(result.current.miningUnitCards[0]?.containerId).toBe('container-1')
    expect(result.current.miningUnitCards[0]?.chartData.total).toBe(10)
  })

...
import {
  ...
  isBitdeer,
  isMicroBT,
} from '@domain/utils/container-utils'
import { getButtonsStates } from '@domain/components/explorer/details-view/details-view-utils'
import { groupTailLogByMinersByType } from '@domain/components/explorer/details-view/miner-controls-card/miner-controls-utils'
import { getContainerState } from '@domain/components/container/helper'
import {
  resetAlarm,
  ...
  switchContainer,
  switchCoolingSystem,
} from '@domain/components/container/container-controls-box/container-controls-box-helpers'
...
vi.mock('@domain/utils/use-update-existed-actions', () => ({
...
vi.mock('@domain/utils/status-utils', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@domain/utils/status-utils')>()
  return {
    ...actual,
    CONTAINER_STATUS: {
      OFFLINE: 'offline',
...
vi.mock('@domain/components/explorer/details-view/details-view-utils', () => ({
...
vi.mock('@domain/components/container/helper', () => ({
...
vi.mock('@domain/components/explorer/details-view/miner-controls-card/miner-controls-utils', () => ({
  groupTailLogByMinersByType: vi.fn(() => ({ mode: 'eco' })),
}))

vi.mock(
  '@domain/components/explorer/details-view/miner-power-mode-selection-buttons/miner-power-mode-selection-buttons',
  () => ({
...
vi.mock('@domain/components/container/content-box/content-box', () => ({
...
vi.mock('@domain/components/alarm/alarm-contents/alarm-contents', () => ({
  AlarmContents: vi.fn(({ alarmsData }) => (
    <div data-testid="alarm-contents" data-count={alarmsData?.length ?? 0} />
  )),
}))

vi.mock('@domain/components/container/enabled-disable-toggle/enabled-disable-toggle', () => ({
...
vi.mock('@domain/components/container/system-status-control-box/system-status-control-box', () => ({
  SystemStatusControlBox: vi.fn(({ data }) => (
    <div data-testid="bitmain-immersion-status" data-id={data?.id} />
  )),
}))

vi.mock('@domain/components/container/container-controls-box/container-controls-box-helpers', () => ({

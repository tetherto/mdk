import { FALLBACK, formatErrors, UNITS } from '@primitives'
...
export const getHeatmapDisplayValue = ({
  ...
  if (error || !miner) return FALLBACK

  if (mode === HEATMAP_MODE.HASHRATE) {
    if (hashRate == null || hashRate === 0) return FALLBACK
    const { value } = getHashrateUnit(hashRate)

    return value !== null ? value : '-'
  }

  return temperature != null ? Math.round(temperature).toString() : FALLBACK
}

...
import { DataRow } from '@domain/components/container/data-row/data-row'
import { GenericDataBox } from '../generic-data-box/generic-data-box'

vi.mock('@domain/components/container/data-row/data-row', () => ({
...
describe('GenericDataBox', () => {
  ...
  it('uses fallbackValue for undefined values', () => {
    ...
    expect(DataRow).toHaveBeenCalledWith(expect.objectContaining({ value: '--' }), undefined)
  })
  ...
  it('prefers units over unit', () => {
    ...
    expect(DataRow).toHaveBeenCalledWith(expect.objectContaining({ units: '°C' }), undefined)
  })

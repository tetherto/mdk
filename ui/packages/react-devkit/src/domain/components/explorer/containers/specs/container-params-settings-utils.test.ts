...
import { isBitdeer, isMicroBT } from '@domain/utils/container-utils'
import { getBitdeerParameterSettingsData } from '@domain/components/explorer/containers/bitdeer'
...
import { getMicroBTThresholdSettingsData } from '@domain/components/explorer/containers/micro-bt/settings/micro-bt-utils'

vi.mock('@domain/utils/container-utils', () => ({
  isBitdeer: vi.fn((type) => type === 'bitdeer' || type === 'bitdeer-immersion'),
  isMicroBT: vi.fn((type) => type === 'microbt' || type === 'microbt-alpha'),
}))

vi.mock('@domain/components/explorer/containers/bitdeer', () => ({
...
vi.mock('@domain/components/explorer/containers/micro-bt/settings/micro-bt-utils', () => ({
...
describe('./container-params-settings-utils', () => {
  ...
  const mockMicroBTAlphaDevice: Device = {
    id: 'microbt-alpha-1',
    type: 'microbt-alpha',
    status: 'active',
    last: {
  ...
  describe('getContainerParametersSettings', () => {
    ...
    it('returns MicroBT settings for microbt-alpha type', () => {
      const result = getContainerParametersSettings(mockMicroBTAlphaDevice)

      expect(result).toBeDefined()
      expect(result).toHaveProperty('runningSpeed')
      expect(isMicroBT).toHaveBeenCalledWith('microbt-alpha')
    })

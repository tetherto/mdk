...
import { getDeviceModel } from '@domain/utils/power-mode-utils'
import type { TimelineItemData } from '../../../../alarm/alarm-row/alarm-row'
import { ContainerControlsBox } from '@domain/components/container'
import { BatchContainerControlsCard } from '../batch-container-controls-card'

vi.mock('@domain/utils/power-mode-utils', () => ({
  getDeviceModel: vi.fn(),
}))

vi.mock('@domain/components/container', () => ({
...
const setSelectedContainers = (containers: Record<string, Device>) => {
  // Reset, then re-add each container so the store reflects the desired map.
  devicesStore.getState().setResetSelections()
  for (const device of Object.values(containers)) {
    devicesStore.getState().selectContainer(device as never)
  }
}

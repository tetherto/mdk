...
import { getNumberSelected, isContainerControlNotSupported } from '@domain/utils/container-utils'
...
const setSelectedSockets = (containers: Record<string, { sockets: any[] }>) => {
  devicesStore.getState().setSelectedSockets(containers as never)
}
...
describe('ContainerControlsCard', () => {
  ...
  describe('Power on action', () => {
    ...
    it('enqueues a SWITCH_SOCKET submission with correct type and action when confirmed', () => {
      renderComponent()
    ...
    it('enqueues a submission with the correct container tag', () => {
      renderComponent()
      fireEvent.click(screen.getByTestId('power-on-btn'))
      expect(setAddSpy).toHaveBeenCalledWith(expect.objectContaining({ tags: ['tag:CON-BBR-01'] }))
    })

    it('enqueues a socket payload with isOn=true', () => {
      setSelectedSockets({
        'CON-BBR-01': {
      ...
      renderComponent()
      fireEvent.click(screen.getByTestId('power-on-btn'))
      expect(setAddSpy).toHaveBeenCalledWith(expect.objectContaining({ params: [[[0, 1, true]]] }))
    })
  ...
  describe('multiple containers', () => {
    it('enqueues once per container that has changes', () => {
      setSelectedSockets({
        'CON-BBR-01': {
      ...
      renderComponent()
      fireEvent.click(screen.getByTestId('power-on-btn'))
      expect(setAddSpy).toHaveBeenCalledTimes(2)
    })
  })
})

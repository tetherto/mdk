...
import { Button, SimpleTooltip, Spinner } from '@primitives'
...
import type { PendingSubmission } from '../../../../utils/use-update-existed-actions'
import { useUpdateExistedActions } from '../../../../utils/use-update-existed-actions'
...
export const MinerControlsCard = ({
  ...
  const { selectedDevices: storedSelectedDevices } = useDevices()
  const { updateExistedActions } = useUpdateExistedActions()

  const selectedDevices = (storedSelectedDevices as Device[]).filter(
  ...
  const [minerDialogFlow, setMinerDialogFlow] = useState<string | null>(null)

...
import { POSITION_CHANGE_DIALOG_FLOWS } from '@domain/constants/dialog'
...
vi.mock(
  '@domain/components/explorer/details-view/miner-setup-frequency-dropdown/miner-setup-frequency-dropdown',
  () => ({
...
vi.mock('@domain/components/explorer/dialogs', () => ({
...
describe('MinerControlsCard', () => {
  ...
  describe('reboot action', () => {
    it('enqueues a pending submission when Reboot is clicked', () => {
      renderComponent()
    ...
    it('enqueues a submission with REBOOT action type', () => {
      renderComponent()
      fireEvent.click(screen.getByRole('button', { name: /reboot/i }))
  ...
  describe('frequency settings action', () => {
    it('enqueues a SETUP_FREQUENCY_SPEED submission when frequency is toggled', () => {
      renderComponent()
    ...
    it('includes crossThing with CONTAINER type in the submission', () => {
      renderComponent()
      fireEvent.click(screen.getByTestId('freq-btn'))
      expect(setAddSpy).toHaveBeenCalledWith(
  ...
  describe('power mode action', () => {
    ...
    it('does not enqueue a submission when power mode is unchanged', () => {
      setSelectedDevices([{ ...normalMiner, last: { snap: { config: { power_mode: 'low' } } } }])
      renderComponent()
      fireEvent.click(screen.getByTestId('power-mode-btn'))
  ...
  describe('LED actions', () => {
    it('enqueues a SET_LED submission when LEDs on is clicked', () => {
      renderComponent()
    ...
    it('does not enqueue a submission when no LEDs are affected', () => {
      setSelectedDevices([{ ...normalMiner, last: { snap: { config: { led_status: true } } } }])
      renderComponent()
      fireEvent.click(screen.getByRole('button', { name: /leds on/i }))
      expect(setAddSpy).not.toHaveBeenCalled()

...
/**
 * Per-status hex colours for the `tiles` variant. Kept as plain `#RRGGBB` (not
 * the CSS-var power-mode colours) so a faint tinted background can be derived.
 * Used for both the tile text and its background tint.
 */
export const MinersActivityTileColors = {
  [MinerStatuses.OFFLINE]: COLOR.DARK_GREY,
  [MinerStatuses.ERROR]: COLOR.BRICK_RED,
  [MINER_POWER_MODE.SLEEP]: COLOR.SLEEP_BLUE,
  [MINER_POWER_MODE.LOW]: COLOR.YELLOW,
  [MINER_POWER_MODE.NORMAL]: COLOR.GRASS_GREEN,
  [MINER_POWER_MODE.HIGH]: COLOR.PURPLE_HIGH,
  [SOCKET_STATUSES.MINER_DISCONNECTED]: COLOR.DARK_GREY,

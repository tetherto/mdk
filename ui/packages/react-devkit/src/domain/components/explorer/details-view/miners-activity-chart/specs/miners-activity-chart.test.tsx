...
vi.mock(
  '@domain/components/explorer/details-view/miners-activity-chart/miners-activity-chart.const',
  () => {
...
describe('MinersActivityChart', () => {
  ...
  describe('tiles variant', () => {
    it('renders tinted tiles (no indicators) with labels and counts', () => {
      render(<MinersActivityChart variant="tiles" data={{ error: 4 }} />)

      // Tiles replace the indicator dots.
      expect(screen.queryByTestId('indicator')).not.toBeInTheDocument()
      // Labels (offline → "empty" alias) and counts render.
      expect(screen.getByText('empty')).toBeInTheDocument()

...
describe('mapDeviceToMinerRecord (flat /auth/miners row)', () => {
  // The normalised endpoint returns flat rows with no nested `info` / `last.snap`.
  const flatRow = {
    id: 'flat-1',
    code: 'WM-M63SPP-0014',
    container: 'group-1',
    status: 'mining',
    hashrate: { t_5m: 297 },
    lastSeen: 1_781_613_937_160,
    poolConfig: [{ url: 'stratum+tcp://btc.f2pool.com:1314' }],
  } as unknown as Device

  it('does not crash and maps flat fields', () => {
    const result = mapDeviceToMinerRecord(flatRow, POOL_ID_MAP)
    expect(result.id).toBe('flat-1')
    expect(result.unit).toBe('group-1')
    expect(result.status).toBe('mining')
    expect(result.hashrate).toBe(297)
    expect(result.lastSyncedAt).toEqual(new Date(1_781_613_937_160))
    // Flat `poolConfig` is an endpoint array, not an id, so no pool name resolves.
    expect(result.pool).toBeUndefined()
  })

  it('reads a numeric flat hashrate', () => {
    const result = mapDeviceToMinerRecord(
      { id: 'flat-2', code: 'X', hashrate: 0, status: 'not_mining' } as unknown as Device,
      POOL_ID_MAP,
    )
    expect(result.hashrate).toBe(0)
    expect(result.status).toBe('not_mining')
  })
})

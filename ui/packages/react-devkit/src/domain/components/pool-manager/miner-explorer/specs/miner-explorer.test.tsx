...
import type { DataTableRowSelectionState } from '@primitives/index'
...
vi.mock('@domain/utils/use-list-view-filters', () => ({
...
vi.mock('@domain/components/pool-manager/miner-explorer/miner-explorer-utils', () => ({
...
vi.mock('@primitives/index', async (importOriginal) => {
  const original = await importOriginal<typeof import('@primitives/index')>()
  return {
    ...original,
...
vi.mock('@domain/components/pool-manager/miner-explorer/miner-explorer-toolbar', () => ({
...
vi.mock('@domain/components/pool-manager/miner-explorer/miner-explorer-table', () => ({
  MinerExplorerTable: ({
    data,
    loading,
  }: {
...
const renderExplorer = (props: Partial<MinerExplorerProps> = {}) =>
  render(<MinerExplorer {...defaultProps} {...props} />)
...
describe('MinerExplorer', () => {
  ...
  describe('search tags', () => {
    ...
    it('calls onSearchTagsChange prop when toolbar fires tag change', async () => {
      const onSearchTagsChange = vi.fn()
      render(<MinerExplorer {...defaultProps} onSearchTagsChange={onSearchTagsChange} />)
      await act(async () => screen.getByTestId('search-change').click())
      expect(onSearchTagsChange).toHaveBeenCalledWith(['tag1'])
    })

    it('passes data prop directly to the table without client-side search filtering', async () => {
      // Search is handled server-side: MinerExplorer passes `data` unchanged.
      render(
        <MinerExplorer
          {...defaultProps}
          data={[makeDevice('alpha'), makeDevice('beta')]}
        />,
      )
      // Before search tag change — both rows present
      expect(screen.getByTestId('miner-explorer-table')).toHaveAttribute('data-row-count', '2')

      await act(async () => screen.getByTestId('search-change').click())

      // After search tag change — data prop is unchanged (server refetch is the parent's job),
      // so the table still shows all rows passed via prop.
      expect(screen.getByTestId('miner-explorer-table')).toHaveAttribute('data-row-count', '2')
    })

    it('reflects server-filtered results when parent passes fewer rows after search', async () => {
      ...
      // Simulate the parent receiving server-filtered results and passing them down
      rerender(<MinerExplorer {...defaultProps} data={[makeDevice('alpha')]} />)
      expect(screen.getByTestId('miner-explorer-table')).toHaveAttribute('data-row-count', '1')
    })
  })
})

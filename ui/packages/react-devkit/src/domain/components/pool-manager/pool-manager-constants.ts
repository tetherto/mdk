import { COLOR, INDICATOR_COLORS } from '@primitives'
import { MinerStatuses, SITE_OVERVIEW_STATUSES } from '@tetherto/mdk-ui-foundation'
...
/**
 * Friendly verbs for the pool-config voting actions used across sidebar cards.
 *
 * @category misc
 * @domain mining-operations
 * @tier internal
 */
export const POOL_ACTION_LABELS: Record<string, string> = {
  [ACTION_TYPES.REGISTER_POOL_CONFIG]: 'Create pool',
  [ACTION_TYPES.UPDATE_POOL_CONFIG]: 'Update pool',
  [ACTION_TYPES.SETUP_POOLS]: 'Assign miners',
}

/**
 * Human-readable badge text for each live action status.
 *
 * @category misc
 * @domain mining-operations
 * @tier internal
 */
export const ACTION_SIDEBAR_STATUS_LABELS: Record<string, string> = {
  voting: 'Action Submitted',
...
/**
 * Badge shown on another user's voting action awaiting the current user's vote.
 *
 * @category misc
 * @domain mining-operations
 * @tier internal
 */
export const ACTION_SIDEBAR_REQUESTED_BADGE = 'Action Requested'

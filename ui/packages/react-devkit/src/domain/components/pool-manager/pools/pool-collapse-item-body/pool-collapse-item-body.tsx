...
import { useActions, useContextualModal } from '@tetherto/mdk-react-adapter'
...
export const PoolCollapseItemBody = ({ pool }: PoolCollapseItemBodyProps) => {
  ...
  const handleAddEndpoint = (values: PoolEndpointFormValues) => {
    ...
    setAddPendingSubmissionAction({
      ...
      // Client-side display field — whitelisted out of the API payload by
      // toVotingPayload, used only by describePendingActionExpanded.
      editedPoolUrl: newPoolUrl.url,
    })

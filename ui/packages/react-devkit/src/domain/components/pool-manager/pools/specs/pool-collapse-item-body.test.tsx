...
vi.mock('@domain/components/pool-manager/pools/add-pool-endpoint-modal/add-pool-endpoint-modal', () => ({
...
describe('PoolCollapseItemBody', () => {
  ...
  describe('add endpoint modal', () => {
    ...
    it('enqueues a pending submission and notifies on endpoint submit', async () => {
      vi.mocked(useContextualModal).mockReturnValue({
        modalOpen: true,
        handleOpen: mockOpenModal,
        handleClose: mockCloseModal,
    ...
    it('adds new endpoint URL when subject is null', async () => {
      ...
      const submission = setAddSpy.mock.calls[0]?.[0] as {
        params: { data: { poolUrls: { url: string }[] } }[]
      }
      const poolUrls = submission.params[0]!.data.poolUrls

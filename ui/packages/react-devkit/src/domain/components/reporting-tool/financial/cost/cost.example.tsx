/**
 * Runnable example for Cost.
 * All imports must come from "@tetherto/mdk-react-devkit/domain".
 */
import {
...
const MOCK_SUMMARY: CostSummaryResponse = {
  log: MOCK_LOG,
  summary: {
    totalEnergyCostsUSD: MOCK_LOG.reduce((s, e) => s + e.energyCostsUSD, 0),
    totalOperationalCostsUSD: MOCK_LOG.reduce((s, e) => s + e.operationalCostsUSD, 0),
    totalCostsUSD: MOCK_LOG.reduce((s, e) => s + e.totalCostsUSD, 0),
    totalConsumptionMWh: MOCK_LOG.reduce((s, e) => s + e.consumptionMWh, 0),
    avgAllInCostPerMWh:
      MOCK_LOG.reduce((s, e) => s + e.totalCostsUSD, 0) /
      MOCK_LOG.reduce((s, e) => s + e.consumptionMWh, 0),
    avgEnergyCostPerMWh:
      MOCK_LOG.reduce((s, e) => s + e.energyCostsUSD, 0) /
      MOCK_LOG.reduce((s, e) => s + e.consumptionMWh, 0),
    avgBtcPrice: MOCK_LOG.reduce((s, e) => s + e.btcPrice, 0) / MOCK_LOG.length,
  },
}

const MOCK_AVG_ALL_IN: AvgAllInCostDataPoint[] = MOCK_LOG.map((e) => ({
  ts: e.ts,
  revenueUSDPerMWh: (e.allInCostPerMWh ?? 0) * 1.28,
  costUSDPerMWh: e.allInCostPerMWh ?? 0,
}))

export const CostExample = () => {
  const [dateRange, setDateRange] = useState<FinancialDateRange>(buildInitialDateRange)
  const viewModel = useMemo(() => buildCostSummaryViewModel({ data: MOCK_SUMMARY }), [])
  ...
  return (
    <Cost
      ...
      dateRange={dateRange}
      avgAllInCostData={MOCK_AVG_ALL_IN}
      controls={
        <TimeframeControls

...
import { EfficiencyMinerTypeView } from '../miner-type-view'
...
vi.mock('@domain/components/reporting-tool/report-time-frame-selector', () => ({
...
describe('EfficiencyMinerTypeView', () => {
  describe('header', () => {
    it('renders the "Efficiency by Miner Type" title', () => {
      render(<EfficiencyMinerTypeView />)
      expect(screen.getByText('Efficiency by Miner Type')).toBeInTheDocument()
    })
  ...
  describe('loading state', () => {
    it('passes loading=true to ChartContainer when isLoading is true', () => {
      render(<EfficiencyMinerTypeView isLoading />)
  ...
  describe('empty state', () => {
    it('passes empty=true to ChartContainer when isEmpty is true', () => {
      render(<EfficiencyMinerTypeView isEmpty />)
    ...
    it('passes empty=true when chartInput has no series data', () => {
      render(<EfficiencyMinerTypeView chartInput={{ series: [] }} />)
      expect(screen.getByTestId('chart-container')).toHaveAttribute('data-empty', 'true')
    })
  })
    ...
    it('renders the BarChart when data is present and not loading', () => {
      render(
        <EfficiencyMinerTypeView
          chartInput={{ labels: ['S19 XP'], series: [{ label: 'Site Efficiency', values: [21] }] }}
        />,
      )
  ...
  describe('onTimeFrameChange', () => {
    it('calls onTimeFrameChange on mount with the resolved start and end dates', () => {
      ...
      render(<EfficiencyMinerTypeView onTimeFrameChange={onTimeFrameChange} />)
    ...
    it('does not throw when onTimeFrameChange is not provided', () => {
      expect(() => render(<EfficiencyMinerTypeView />)).not.toThrow()
    })
  })
})

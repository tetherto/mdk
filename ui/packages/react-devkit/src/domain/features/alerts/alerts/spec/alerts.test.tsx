...
const { mockAppendIdToTags } = vi.hoisted(() => ({
...
vi.mock('@domain/components/alerts/current-alerts/current-alerts', () => ({
  CurrentAlerts: ({
    devices,
    isLoading,
    localFilters,
...
vi.mock('@domain/components/alerts/historical-alerts/historical-alerts', () => ({
  HistoricalAlerts: ({
    alerts,
    isLoading,
    dateRange,
...
describe('Alerts (feature)', () => {
  let setFilterTagsSpy: ReturnType<typeof vi.spyOn>

  beforeEach(() => {
    devicesStore.getState().setFilterTags([])
    mockAppendIdToTags
      .mockReset()
      .mockImplementation((ids: string[]) => ids.map((id) => `id-${id}`))
    setFilterTagsSpy = vi.spyOn(devicesStore.getState(), 'setFilterTags')
  })

  afterEach(() => {
    setFilterTagsSpy.mockRestore()
    devicesStore.getState().setFilterTags([])
  })
  ...
  describe('current alerts wiring', () => {
    ...
    it('reads filter tags from the devices store', () => {
      devicesStore.getState().setFilterTags(['ip-9'])
      renderComponent()
      expect(screen.getByTestId('ca-filter-tags')).toHaveTextContent('ip-9')
    })

    it('calls setFilterTags when CurrentAlerts emits new tags', () => {
      renderComponent()
      fireEvent.click(screen.getByTestId('ca-update-tags'))
      expect(setFilterTagsSpy).toHaveBeenCalledWith(['ip-1'])
      expect(devicesStore.getState().filterTags).toEqual(['ip-1'])
    })
  ...
  describe('alert click', () => {
    it('calls setFilterTags with appendIdToTags before invoking onAlertClick', () => {
      ...
      expect(setFilterTagsSpy).toHaveBeenCalledWith(['id-device-1'])
      expect(devicesStore.getState().filterTags).toEqual(['id-device-1'])
      expect(onAlertClick).toHaveBeenCalledWith('device-1', 'alert-uuid-1')
    })

    it('does not call setFilterTags before any click happens', () => {
      const onAlertClick = vi.fn()
  ...
  describe('historical alerts wiring', () => {
    ...
    it('shares the alert click handler with HistoricalAlerts', () => {
      ...
      expect(setFilterTagsSpy).toHaveBeenCalledWith(['id-device-2'])

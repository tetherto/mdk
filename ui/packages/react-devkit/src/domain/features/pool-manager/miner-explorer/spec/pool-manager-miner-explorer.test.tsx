...
import { useCheckPerm, useContextualModal } from '@tetherto/mdk-react-adapter'
import type { Device } from '@domain/types'
import { PoolManagerMinerExplorer } from '../pool-manager-miner-explorer'

const { mockNotifyInfo, mockOpenModal, mockCloseModal } = vi.hoisted(() => ({
...
vi.mock('@tetherto/mdk-react-adapter', async (importOriginal) => {
  ...
  return {
    ...
    // useMinerDevices is now called inside PoolManagerMinerExplorer for server-side
    // search. Stub it out so the test doesn't need a QueryClientProvider.
    useMinerDevices: vi.fn(() => ({ data: [], isLoading: false, isFetching: false, error: null, refetch: vi.fn() })),
  }
})
...
vi.mock('@domain/components/pool-manager/miner-explorer', () => ({
  MinerExplorer: ({
...
vi.mock('@domain/components/pool-manager/assign-pool-modal/assign-pool-modal', () => ({
  AssignPoolModal: ({
    isOpen,
    onClose,
    onSubmit,
...
describe('PoolManagerMinerExplorer', () => {
  ...
  describe('handleAssignPoolSubmit', () => {
    ...
    it('enqueues a pending submission with correct payload', async () => {
      renderComponent()
    ...
    it('enqueues a pending submission with selected device ids in query', async () => {
      renderComponent()
      fireEvent.click(screen.getByTestId('modal-submit-btn'))
      await waitFor(() => {
        expect(setAddSpy).toHaveBeenCalledWith(
    ...
    it('calls notifyInfo after enqueuing', async () => {
      renderComponent()
      fireEvent.click(screen.getByTestId('modal-submit-btn'))
      await waitFor(() => {

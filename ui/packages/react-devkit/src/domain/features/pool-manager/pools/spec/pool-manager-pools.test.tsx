...
import { usePoolConfigs } from '@domain/components'
import type { PoolSummary } from '@domain/components/pool-manager/types'
...
vi.mock('@domain/components', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@domain/components')>()
  return {
    ...actual,
...
vi.mock(
  '@domain/components/pool-manager/pools/pool-collapse-item-header/pool-collapse-item-header',
  () => ({
...
vi.mock('@domain/components/pool-manager/pools/pool-collapse-item-body/pool-collapse-item-body', () => ({
  PoolCollapseItemBody: ({ pool }: { pool: PoolSummary }) => (
    <div data-testid="pool-collapse-body" data-pool-name={pool.name} />
  ),
}))

vi.mock('@domain/components/pool-manager/pools/add-pool-modal/add-pool-modal', () => ({

...
import type { ToastPosition, ToastVariant } from '@primitives'
import { Toast, Toaster } from '@primitives'
...
const ToastContainer = ({ position = 'top-left' }: { position?: ToastPosition }): ReactElement => {
...
/**
 * Tears down the static toast portal (React root, DOM node, in-memory queue).
 * Used by tests so auto-close timers cannot flush after the environment is destroyed.
 */
export const resetToastPortalForTests = (): void => {
  if (toastRoot) {
    toastRoot.unmount()
    toastRoot = null
  }
...
const openNotification = (
  ...
  addToast({
    ...
    position,
    onClose() {
      notificationStore.getState().decrement()
    },
  })
}

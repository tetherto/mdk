...
import { getContainerSpecificStats, getCoolingSystem } from '@domain/utils/device-utils'
...
const GET_CONTAINER_NAME_TEST_ARGS = {
  ...
  microBT: { type: 'container-mbt-alpha', container: 'microbt-1' },
}

vi.mock('@domain/utils/device-utils', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@domain/utils/device-utils')>()
  return {
    ...actual,
...
describe('container utils', () => {
  ...
  describe('isMicroBTAlpha', () => {
    it('should detect MicroBT Alpha containers', () => {
      expect(isMicroBTAlpha('container-mbt-alpha')).toBe(true)
      expect(isMicroBTAlpha('container-mbt-alpha-1')).toBe(true)
      expect(isMicroBTAlpha('microbt-beta')).toBe(false)
      expect(isMicroBTAlpha('bitdeer-1')).toBe(false)
    })
  })
  ...
  describe('getContainerName edge cases', () => {
    ...
    it('should handle MicroBT Beta', () => {
      expect(getContainerName('microbt-1', 'container-mbt-beta')).toBe('MicroBT 1 Beta')
    })
    ...
    it('should handle two-part names when three-part pattern does not match', () => {
      expect(getContainerName('M221-1')).toBe('M221 1')
    })
    ...
    it('should handle MicroBT Alpha with full type', () => {
      const result = getContainerName('microbt-2', 'container-mbt-alpha')
  ...
  describe('type detection edge cases', () => {
    ...
    it('should handle container type patterns', () => {
      ...
      expect(isMicroBT('container-mbt-alpha')).toBe(true)

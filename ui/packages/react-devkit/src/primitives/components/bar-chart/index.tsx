...
// chart.js + react-chartjs-2 (and the datalabels plugin) are code-split into a
// lazy chunk so importing this component does not pull the chart engine into a
// consumer's initial bundle.
        ...
        datalabels: showDataLabels
          ? {
              display: (context: DataLabelContext): boolean => {
                const { chart, dataset, datasetIndex, dataIndex } = context
    ...
    const chartData = useMemo(() => {
      const datasets = data.datasets?.map((ds: any, i: number) => {
        ...
        if (bgIsFunction) {
          ...
          backgroundColor = ((context: BarGradientContext) =>
            makeBarGradient(
              context,
              ds.backgroundColor[context.dataIndex] ?? solidColor,
            )) as unknown as string
        } else {
          backgroundColor = ((context: { chart: ChartJS }) =>
            makeBarGradient(context, solidColor)) as unknown as string
        }
    ...
    return (
      <div ref={ref} className={cn('mdk-bar-chart', className)} style={{ height }}>
        <Suspense fallback={null}>
          <BarCanvas data={chartData} options={mergedOptions} showDataLabels={showDataLabels} />
        </Suspense>
      </div>
    )
  },

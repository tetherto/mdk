...
export type DataTableProps<I = unknown> = {
  ...
  /**
   * Default sorting applied when the table is first mounted (uncontrolled mode).
   * Ignored when `sorting` is provided.
   * @default []
   */
...
/**
 * TanStack Table may invoke state handlers while React is still mounting the
 * table surface. Deferring avoids Radix useControllableState updates before mount.
 */
const useDeferredTableOnChange = <T,>(onChange: OnChangeFn<T>): OnChangeFn<T> =>
  useCallback(
    (updater: Updater<T>) => {
      queueMicrotask(() => onChange(updater))
    },
    [onChange],
  )

/**
 * Generic typed data table built on TanStack React Table. Supports
 * pagination, sorting, expansion, row selection and custom column renderers.
 *
 * @category tables
 * @domain generic
 * @tier agent-ready
 */
export const DataTable = <I = unknown,>({
  data,
  ...
  const handleSortingChange = useDeferredTableOnChange(setSorting)
  const handlePaginationChange = useDeferredTableOnChange(setPagination)
  const handleRowSelectionChange = useDeferredTableOnChange(setRowSelection)
  const handleExpandedChange = useDeferredTableOnChange(setExpandedRows)
  ...
  const tableBackend = useReactTable({
    ...
    // Sorting — always wired (unlike pagination/selection/expansion): table state and
    // controlled `sorting` props stay valid even when every column sets enableSorting: false.
    ...
    ...(enableRowExpansion ? { onExpandedChange: handleExpandedChange } : {}),
  ...
  // Keep footer when row count exceeds default page size so page-size control stays
  // available after switching to a larger page size (e.g. 20) that fits all rows on one page.
  const showPagination =
    enablePagination &&
    hasData &&
    (tableBackend.getPageCount() > 1 || data.length > DEFAULT_PAGE_SIZE)

...
describe('dataTable with row selection', () => {
  ...
  it('calls onSelectionsChange when selection changes', async () => {
...
describe('dataTable with pagination', () => {
  ...
  it('hides pagination when all rows fit on one page', () => {
    const fewRows: Row[] = Array.from({ length: 3 }, (_, i) => ({
      id: String(i + 1),
      name: `Item ${i + 1}`,
      value: i + 1,
    }))
    const { container } = render(
      <DataTable data={fewRows} columns={basicColumns} enablePagination />,
    )

    expect(container.querySelector('.mdk-table__pagination-section')).not.toBeInTheDocument()
  })

  it('keeps pagination footer at page size 20 when row count is above default page size', () => {
    const { container } = render(
      <DataTable
        data={manyRows}
        columns={basicColumns}
    ...
    expect(container.querySelector('.mdk-table__pagination-section')).toBeInTheDocument()
  })
})

describe('dataTable with row expansion', () => {
  it('renders expand button and expanded content when enableRowExpansion is true', async () => {
    render(
      <DataTable
        data={sampleData}
        columns={basicColumns}

...
describe('gaugeChart', () => {
  ...
  it('renders one segment per arcsLength entry (overriding nrOfLevels)', () => {
    const { container } = render(
      <GaugeChart percent={0.7} id="gauge" nrOfLevels={5} arcsLength={[0.7, 0.3]} />,
    )
    expect(container.querySelectorAll('svg > g > path')).toHaveLength(2)
  })

  it('hides the needle and hub when hideNeedle is true', () => {
    const { container } = render(<GaugeChart percent={0.5} id="gauge" hideNeedle />)

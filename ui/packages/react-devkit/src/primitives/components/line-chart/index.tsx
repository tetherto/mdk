 ...
 *
 * Prefer wrapping in {@link ChartContainer} or {@link LineChartCard} for
 * title, legend, range selector, and loading / empty states.
 *
 * @category charts
 *
 * @example
 * ```tsx
 * <LineChart data={{ datasets: [{ label: 'Hashrate', data: points }] }} height={320} />
 * ```
 */
const LightWeightLineChart = ({
  ...
  // lightweight-charts is loaded lazily so it lands in its own async chunk and
  // is kept out of a consumer's initial bundle until a line chart renders.
  const [createChart, setCreateChart] = useState<
    typeof import('lightweight-charts').createChart | null
  >(null)
  useEffect(() => {
    let active = true
    void import('lightweight-charts').then((m) => {
      if (active) {
        setCreateChart(() => m.createChart)
      }
    })
    return () => {
      active = false
      ...
      if (chartRef) {
        handleChartRef(
          createChart(
            ...
            chartOptions as Parameters<typeof import('lightweight-charts').createChart>[1],
          ),
        )
      }
    }
  ...
  }, [createChart, chartRef, data.datasets, data, priceFormatter, yTicksFormatter, chartOptions, beginAtZero, showPointMarkers])
...
/**
 * Customisable Chart.js line chart with built-in zoom, tooltip, and
 * legend. Data is passed via props; the component does no fetching.
 *
 * @category charts
 * @domain generic
 * @tier agent-ready

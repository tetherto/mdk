...
import { CHART_COLORS, COLOR, UNITS } from '@tetherto/mdk-react-devkit/primitives'
import {
  autoscaleProvider,
  buildTooltipHTML,
  getTooltipPosition,
...
describe('buildTooltipHTML', () => {
  ...
  it('appends unit to value', () => {
    ...
    expect(result).toContain(`100 ${UNITS.HASHRATE_TH_S}`)
  })
  ...
  it('returns empty string when no visible series or crosshair point', () => {
    const result = buildTooltipHTML({
      seriesData: new Map(),
  ...
  it('includes series when visible is undefined (default visible)', () => {
    const mockSeries = new Map() as any
    const mockDataset = {
    ...
    seriesToDatasetMap.set(mockSeriesApi, mockDataset)
    mockSeries.set(mockSeriesApi, { time: 1704067200, value: 38_000_000_000_000 })

    const result = buildTooltipHTML({
      seriesData: mockSeries,
      seriesToDatasetMap,
      yTicksFormatter: () => `38 ${UNITS.HASHRATE_EH_S}`,
    })

    expect(result).toContain('Network Hashrate')
    expect(result).toContain(`38 ${UNITS.HASHRATE_EH_S}`)
  })

  it('does not duplicate unit when yTicksFormatter already includes it', () => {
    const mockSeries = new Map() as any
    const mockDataset = {
    ...
    seriesToDatasetMap.set(mockSeriesApi, mockDataset)
    mockSeries.set(mockSeriesApi, { time: 1704067200, value: 18_727 })

    const result = buildTooltipHTML({
      seriesData: mockSeries,
      seriesToDatasetMap,
      yTicksFormatter: (v) => `${v} ${UNITS.HASHRATE_PH_S}`,
      unit: UNITS.HASHRATE_PH_S,
    })

    expect(result).toContain(`18727 ${UNITS.HASHRATE_PH_S}`)
    expect(result).not.toContain(`${UNITS.HASHRATE_PH_S} ${UNITS.HASHRATE_PH_S}`)
  })

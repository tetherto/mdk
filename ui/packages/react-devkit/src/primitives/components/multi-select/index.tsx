...
import { runOnEnterOrSpace } from './keyboard-activation'
...
export const MultiSelect = forwardRef<HTMLButtonElement, MultiSelectProps>(
  (
    ...
    const stopChipClickFromOpeningPopover = (event: MouseEvent<HTMLElement>) => {
      event.stopPropagation()
    }

    const onChipRemove = (event: MouseEvent<HTMLSpanElement>, optionValue: string) => {
      stopChipClickFromOpeningPopover(event)
      event.preventDefault()
    ...
    const onClearAllClick = (event: MouseEvent<HTMLSpanElement>) => {
      stopChipClickFromOpeningPopover(event)
      event.preventDefault()
      if (disabled) return
      clearAll()
    ...
    const onClearAllKeyDown = (event: KeyboardEvent<HTMLSpanElement>): void => {
      runOnEnterOrSpace(event, () => {
        if (disabled) {
          return
        }
    ...
    const handleOptionFocus = (event: FocusEvent<HTMLDivElement>) => {
    ...
    const activateOption = (idx: number): void => {
    ...
    const handleOptionClick = (event: MouseEvent<HTMLDivElement>) => {
      event.preventDefault()
      activateOption(Number(event.currentTarget.dataset.index))
    }
                ...
                {visibleChips.map((chip) => (
                  <span key={chip.value} className="mdk-multi-select__token">
                    ...
                    <span
                      ...
                      onClick={(event) => onChipRemove(event, chip.value)}
                      onKeyDown={(event) => onChipClearKeyDown(event, chip.value)}
                    >
                      <CloseIcon />
                    </span>
                  </span>
              ...
              {_map(options, (opt, idx) => {
                ...
                return (
                  <div
                    ...
                    tabIndex={isOptionDisabled ? -1 : idx === activeIdx ? 0 : -1}
                    className="mdk-multi-select__option"
                    onFocus={handleOptionFocus}
                    onClick={isOptionDisabled ? undefined : handleOptionClick}
                    onKeyDown={isOptionDisabled ? undefined : handleOptionKeyDown}
                  >
                    <Checkbox
                      checked={isSelected}

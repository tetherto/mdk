...
// Mock react-dom (React 19 removed the legacy `render` export — only the
// concurrent `createRoot` API ships from `react-dom/client`).
// We forward the real exports first so DOM-only helpers like `createPortal`
// keep working in component tests (e.g. Sidebar overlays).
...
globalThis.getPrefixCls = vi.fn(
  (suffix, customizePrefixCls) => customizePrefixCls || `ant-${suffix}`,
)
globalThis.ConfigContext = mockContext

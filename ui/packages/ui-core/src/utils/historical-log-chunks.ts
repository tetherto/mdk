 ...
 *
 * Ported from Moria's `useFetchHistoricalLogsPaginatedData` /
 * `breakTimeIntoIntervals`, but framework-agnostic: the per-window fetcher is
 * injected, so the React adapter can wrap this in `useQuery` while ui-core
 ...
 * or inverted. Mirrors Moria's `breakTimeIntoIntervals`.
 *
 * @category alerts
 */
 ...
 * appended. Mirrors Moria's `updateHistoricalData`.
 *
 * @category alerts
 */
 ...
 * newest); individual window failures are swallowed (matches Moria) so one bad

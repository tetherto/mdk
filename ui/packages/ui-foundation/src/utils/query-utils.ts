 ...
 * `[[row, ...], [row, ...]]` — one inner array per Kernel). Null-safe on both
 * levels: a missing envelope and null per-Kernel groups/rows are dropped
 * (workers can skip or fail a poll at any time).
 */
export const flattenKernelEnvelope = <T>(
  envelope: ReadonlyArray<readonly (T | null | undefined)[] | null | undefined> | null | undefined,
): T[] =>
  (envelope ?? [])
    .filter((group): group is readonly (T | null | undefined)[] => group != null)
    .flat()
